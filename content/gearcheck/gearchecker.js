...
async function submit() {
  ...
  new Response(document.getElementById("geardata").files[0]).json().then(
    (userGear) => {
      ...
      const misDiv = document.createElement("details");
      const misTitle = document.createElement("summary");
      const misContainer = document.createElement("div");
      ...
      const missing = missingGear(info, userGear);
      ...
      for (let i = 0; i <= 5; i++) {
        const starDiv = document.createElement("details");
        const starTitle = document.createElement("summary");
    ...
    jsonlist = jsonlistunfiltered.filter(
      (x) =>
        !(
          ...
          x.Season > info.ver.substring(0, 2) ||
...
function displayGear(info, missing, div) {
  ...
  for (j = 0; j < 3; j++) {
    ...
    for (i = 0; i < diff.length; i++) {
      ...
      const item = makeItem(image, name);
